...
import type { CheckErc20BalanceResult } from '../types';
...
export const checkErc20Balance = async ({
  ...
  if (tokenBalance.lt(requiredTokenAmount)) {
    return {
      success: false,
      reason: `pkpEthAddress (${pkpEthAddress}) has insufficient balance of wrapped BTC (${tokenAddress}). Wanted ${requiredTokenAmount}, but only have ${tokenBalance}`,
      tokenAddress,
      requiredTokenAmount,
      tokenBalance,
    };
